...
router.post('/comment',authMiddleware, async (req, res) => {
    ...
    const { postId, commentUserId, commentDesc } = req.body;
    ...
    isExits = await Comment.findOne({ postId });
    if (!isExits) {
      res.status(403).send({
    ...
    await StudyComment.create({ commentId, postId, commentDesc, commentUserId, commentDate : currentDate });
    res.send({ commentId });
  });


//댓글쓰기 - authMiddleware 빼고 postman에 실험
router.post('/comment', async (req, res) => {
    const { postId,  commentUserId } = req.body //요청:post 방식으로 넘어오는 postId를 파라미터 담기
    const { user } = res.locals //응답: 뷰를 렌더링하는 기본 콘텍스트 포함

    const userId = user['userId']  //user 스키마와 연결
